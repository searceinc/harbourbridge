...
```sh
harbourbridge -driver=postgre
```

It is assumed that PostgresSQL database supports information schema tables and _PGHOST_, _PGPORT_, _PGUSER_, _PGDATABASE_ environments variables are set. Password can be specified either in the _PGPASSWORD_ environment variable or need to enter password when prompted by harbourbridge.
