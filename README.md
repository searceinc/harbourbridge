...
using data from an existing PostgreSQL / MySQL database. The tool ingests from database / dump file output, automatically builds a Spanner schema, and creates a new Spanner
database populated with data from the selected approach.
...
associated with this project, convert the MySQL / PostgreSQL schema for `mydb` to a
...
Spanner database will have a name of the form `{DRIVER}_{DATE}_{RANDOM}`, where`{DRIVER}`is the drivername specified,`{DATE}`is today's date, and`{RANDOM}` is a random suffix for uniqueness.
...
`-driver` Specifies the driver to use for schema and data conversion. Supported drivers are _'postgres'_, _'pgdump'_, _'mysql'_ and _'mysqldump'_. It is mandatory to specify a driver.
...
Driver types supported by Harbourbridge:
...
Please refer below mentioned links for example usage of MySQL and PostgresSQL database:

- [MySQL example usage](mysql/README.md#example-usage)
- [PostgresSQL example usage](postgres/README.md#example-usage)

## Schema Conversion

Please refer below mentioned links for details on schema conversion of MySQL and PostgresSQL database:

- [MySQL schema conversion](mysql/README.md#schema-conversion)
- [PostgresSQL schema conversion](postgres/README.md#schema-conversion)
...
for databases larger than a couple of GB. Please refer below mentioned links for details on data conversion of MySQL and PostgresSQL database:

- [MySQL schema conversion](mysql/README.md#data-conversion)
- [PostgresSQL schema conversion](postgres/README.md#data-conversion)
...
### 1. Verify dump configuration

First, check that pg_dump/mysqldump is correctly configured to connect to your PostgreSQL/MySQL
database. Also note the below mentioned points:

- pg_dump uses the same options as psql to connect to your
...
- mysqldump uses the same options as mysql to connect to your
...
Access to a MySQL database is configured using the following
...
It is also possible to configure access via mysqldump's command-line options
`--host`, `--port`, and `--user`.
...
[pg_dump documentation](https://www.postgresql.org/docs/9.3/app-pgdump.html) and [mysqldump documentation](https://dev.mysql.com/doc/refman/8.0/en/mysqldump.html) for details about formats.
