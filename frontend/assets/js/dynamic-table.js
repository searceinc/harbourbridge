...
import {downloadSession, getGlobalDataTypeList , initSchemaScreenTasks ,downloadReport ,globalEditHandler,  reportExpandHandler , downloadDdl , searchTable, setGlobalDataType, ddlExpandHandler, summaryExpandHandler } from '../../app/view/schema-conversion/schema-conversion.js'
...
function addConversionEvent() {
    ...
    jQuery("#download-schema").on('click' , downloadSession)

    // download button event listener 
    jQuery("#download-ddl").on('click' , downloadDdl)
    jQuery("#download-report").on('click' ,downloadReport )

    // edit global data type event listener
    jQuery("#editButton").on('click' ,globalEditHandler )

    // expnad button event listener

    jQuery("#reportExpandButton").on('click' , reportExpandHandler )
    jQuery("#ddlExpandButton").on('click' , ddlExpandHandler)
    jQuery("#summaryExpandButton").on('click' , summaryExpandHandler )

    //next button eventlistener

    jQuery("#data-type-button").on('click' , setGlobalDataType )
...
export { createDdlFromJson , createSummaryFromJson ,createSourceAndSpannerTables,addConversionEvent }
