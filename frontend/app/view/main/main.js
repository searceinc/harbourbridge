...
const editSpannerColumnName = (tableNumber, tableColumnNumber, tablePkArray, tableOriginalColNames, notPrimaryArray) => {
  ...
  if (notPrimaryArray[tableColumnNumber] === false) {
    $editColumnName.find('.column.left.key-margin.key-click > img').attr('src', './assets/images/Icons/ic_vpn_key_24px.svg');
    $editColumnName.find('.column.left.key-margin.key-click > sub').html(currSeqId);
    $editColumnName.find('.column.left.key-margin.key-click').addClass('keyActive');
    $editColumnName.find('.column.left.key-margin.key-click').removeClass('keyNotActive');
  }
}
...
const editSpannerConstraint = (editColumn, tableNumber, tableColumnNumber, columnsNotNullConstraint) => {
  ...
  constraintHtml = "<select id=" + constraintId + " multiple size='0' class='form-control spanner-input table-select' >"
    + notNullFound
    + "</select>";
...
<tbody class='acc-table-body'></tbody>`);
  ...
  jQuery('#src-sp-table' + tableNumber).DataTable({ "paging": false });
...
const onLoadDatabase = async (dbType, dumpFilePath) => {
  ...
  if (requestCode != 200) {
    hideSpinner();
    showSnackbar(reportDataResp, ' red-bg');
    ...
    if (Object.keys(jsonReportDataResp.SpSchema).length == 0) {
      showSnackbar("Please select valid file", " red-bg");
      jQuery("#loadConnectButton").attr("disabled", "disabled");
      return;
    }
    else {
    ...
    .then(async function (res) {
      if (res.ok) {
        ...
        if (Object.keys(jsonResponse.SpSchema).length == 0) {
          showSnackbar('Please select valid session file', ' red-bg');
    ...
    await fetch('/setparent?table=' + tableName + '&update=' + true)
      .then(async function (res) {
        ...
        if (table.Fks != null && table.Fks.length != 0) {
          ...
          let $newFkCard = jQuery(".fk-card.template").clone().removeClass("template");
          $newFkCard.find('.fk-font').attr('href', '#foreignKey' + index);
          $newFkCard.find('fieldset').attr('id', 'radioBtnArea' + index);
          $newFkCard.find('.fk-font').html('Foreign Keys');
          $newFkCard.find('.collapse.fk-collapse').attr('id', 'foreignKey' + index);
          ...
          jQuery('#' + index).find(".collapse.fk-collapse .mdc-card.fk-content").html($newFkCard.find(".collapse.fk-collapse .mdc-card.fk-content").html());
          for (let p = 0; p < table.Fks.length; p++) {
          ...
          jQuery('#' + index).find('.fk-card').addClass('template');
        }
      })
  }
  else {
    ...
    .then(async function (response) {
      if (response.ok) {
        ...
        if (jsonObj.SpSchema[tableName].Indexes != null && jsonObj.SpSchema[tableName].Indexes.length != 0) {
          ...
          jQuery('#' + tableNumber).find('.index-acc-table.fk-table').css('visibility', 'hidden');
          jQuery('#' + tableNumber).find('.index-acc-table.fk-table').addClass('important-rule-0');
          jQuery('#' + tableNumber).find('.index-acc-table.fk-table').removeClass('important-rule-100');
        }
      }
      else {
        return Promise.reject(response);
...
const createNewSecIndex = (id) => {
  ...
  for (let k = 0; k < columnsLength; k++) {
    ...
    newIndexColumnRow.find("input").click(function () {
      if (jQuery(this).is(':checked')) {
        orderId = orderId + 1;
        jQuery(this).closest(".newIndexColumnList").find(".order-id").html(orderId);
        jQuery(this).closest(".newIndexColumnList").find(".order-id").css("visibility", "visible");
        keysList.push({ "Col": jQuery(this).closest(".newIndexColumnList").find(".columnName").html(), "Desc": false });
      }
      else {
        let deletedOrderId = jQuery(this).closest(".newIndexColumnList").find(".order-id").html();
        jQuery(this).closest(".newIndexColumnList").find(".order-id").css("visibility", "hidden");
        ...
        for (let x = 0; x < columnsLength; x++) {
          if (jQuery("#indexColumnRow" + x).find("input").is(":checked")) {
            ...
            let currOrderId = jQuery("#indexColumnRow" + x).find(".order-id").html();
            jQuery(this).closest(".newIndexColumnList").find(".order-id").css("visibility", "hidden");
            if (currOrderId > deletedOrderId) {
              jQuery("#indexColumnRow" + x).find(".order-id").html(currOrderId - 1);
              jQuery(this).closest(".newIndexColumnList").find(".order-id").css("visibility", "none");
            }
          }
        }
        for (let x = 0; x < columnsLength; x++) {
...
const fetchIndexFormValues = async (name, uniqueness) => {
  if (keysList.length == 0) {
    showSnackbar("Please select atleast one key to create a new index", " red-bg");
    return;
  }
  ...
  await fetch("/add/indexes?table=" + table.Name, {
    ...
    .then(async function (res) {
      if (res.ok) {
        ...
        let table = jsonObj.SpSchema[srcTableName[tableNumber]];
        let indexKeys;
        jQuery("#" + tableNumber).find('.index-acc-table.fk-table').css('visibility', 'visible');
        jQuery('#' + tableNumber).find('.index-acc-table.fk-table').addClass('important-rule-100');
        jQuery('#' + tableNumber).find('.index-acc-table.fk-table').removeClass('important-rule-0');
...
const resumeSession = async (driver, path, dbName, sourceDb) => {
  ...
  readTextFile(filePath, async function (error, text) {
    if (error) {
      showSnackbar('File does not exist', ' red-bg');
    }
    else {
 ...
 */
const homeScreen = () => {
  jQuery('#app').load('./app/view/main/main-screen.html');
}